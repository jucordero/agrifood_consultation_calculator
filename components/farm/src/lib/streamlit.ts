...
export class Streamlit {
	...
	private static onRenderMessage = <ArgType = any>(data: {
		...
		const dataframeArgs =
			data['dfs'] && data['dfs'].length > 0 ? Streamlit.argsDataframeToObject(data['dfs']) : {};
	...
	private static argsDataframeToObject = (argsDataframe: ArgsDataframe[]): object => {
		const argsDataframeArrow = argsDataframe.map(({ key, value }: ArgsDataframe) => [
			key,
